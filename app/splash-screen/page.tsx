...
// Alternative version with fade-out transition
export function SplashScreenWithFadeOut() {
  const { authenticated, user } = usePrivy();
  const router = useRouter();
  const [hasMinTimeElapsed, setHasMinTimeElapsed] = useState(false);
  ...
  }, [authenticated, user, hasMinTimeElapsed]);

  useEffect(() => {
    if (shouldNavigate) {
      // Add a small delay for fade-out effect
  ...
  }, [shouldNavigate, authenticated, user, router]);

  return (
    <motion.div
      className="bg-background h-dvh w-dvw flex flex-col items-center justify-center"
      initial={{ opacity: 1 }}
      animate={{ opacity: shouldNavigate ? 0 : 1 }}
